...
fn update_object_group_test() {
    ...
    let third_update_request = UpdateObjectGroupRequest {
        group_id: object_group_rev_2.id.to_string(),
        name: object_group_name.to_string(),
        description: updated_description.to_string(),
        collection_id: random_collection.id.to_string(),
        object_ids: object_ids.clone(),
...
fn delete_object_group_test() {
    ...
    let get_rev_0_response = db
        .get_object_group_by_id(&GetObjectGroupByIdRequest {
            group_id: object_group_rev_0.id.to_string(),
            collection_id: random_collection.id.to_string(),
        })
        .unwrap();
    ...
    let get_deleted_rev_0_response = db.get_object_group_by_id(&GetObjectGroupByIdRequest {
        group_id: object_group_rev_0.id.to_string(),
        collection_id: random_collection.id.to_string(),
    });
    ...
    match get_deleted_rev_0_response {
        ...
        Err(err) => {
            match &err {
                ArunaError::DieselError(diesel_err) => match diesel_err {
                    Error::NotFound => Ok(()),
                    _ => Err(err),
                },
                _ => Err(err),
    ...
    let get_deleted_rev_1_response = db.get_object_group_by_id(&GetObjectGroupByIdRequest {
        group_id: object_group_rev_1.id.to_string(),
        collection_id: random_collection.id.to_string(),
    });
