...
use crate::{
    context::Context, error::ArunaError, models::requests::{GetEventsRequest, GetEventsResponse}
};
...
impl Request for GetEventsRequest {
    ...
    async fn run_request(
        ...
        let response = tokio::task::spawn_blocking(move || {
            ...
            let event_ids = store
                .get_events_subscriber(&mut wtxn, self.subscriber_id.0, self.acknowledge_from.map(|x| x.0))
                .map_err(|e| {
            ...
            for event in event_ids.iter() {
                ...
                let dyn_event: Box<dyn WriteRequest> = bincode::deserialize(&event.transaction).map_err(|e| {
