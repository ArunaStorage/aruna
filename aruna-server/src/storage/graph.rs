use crate::{
    error::ArunaError,
    models::{EdgeType, NodeVariant, RawRelation},
};
...
pub fn load_graph(
    ...
    documents.iter(rtxn)?.try_for_each(|entry| {
        let (_id, obkv) = entry.map_err(|e| {
            ArunaError::ServerError(format!("{e}"))
        })?;
        ...
        let value = serde_json::from_slice::<serde_json::Number>(node_type).map_err(|e| {
            ArunaError::ConversionError{from: "&[u8]".to_string(), to: "serde_json::Number".to_string()}
        })?;
    ...
    relations.iter(&rtxn)?.try_for_each(|entry|{
        let (_idx, relation) = entry.map_err(|e| {
            ArunaError::ServerError(format!("{e}"))
        })?;
        graph.add_edge(relation.source.into(), relation.target.into(), relation.edge_type);
        Ok::<(), ArunaError>(())
    })?;
    Ok(graph)
}
