                    ...
                    Some(coll) => {
                        ...
                        for elem in coll {
                            ...
                            let stats = collection_stats
                                .filter(
                                    crate::database::schema::collection_stats::dsl::id.eq(elem.id)
                                )
                            ...
                            let version = match elem.version_id {
                                Some(vid) =>
                                    collection_version
                                        .filter(
                                            crate::database::schema::collection_version::dsl::id.eq(
                                                vid
                                            )
                                        )
                ...
                elements
                    ...
                    .map(|(coll, keyvalue, stats, req_label, vers)| {
                        ...
                        let map_version = match vers {
                            Some(v) =>
                                Some(
                                    CollectionVersiongRPC::SemanticVersion(Version {
