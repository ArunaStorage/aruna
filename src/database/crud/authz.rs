...
impl Database {
    ...
    pub fn get_or_add_pub_key(&self, pub_key: String) -> Result<i64, ArunaError> {
        use crate::database::schema::pub_keys::dsl::*;
        use diesel::result::Error as dError;
        let result = self.pg_connection.get()?.transaction::<i64, dError, _>(|conn| {
            let pkey = pub_keys
            ...
            if let Some(pkey_noption) = pkey {
                ...
                Ok(insert_into(pub_keys).values(&new_pkey).returning(id).get_result::<i64>(conn)?)
            }
        })?;
        Ok(result)
    }
    ...
    pub fn get_checked_user_id_from_token(
        ...
        let creator_uid = self.pg_connection
            .get()?
                ...
                if req_ctx.resource_type == Resources::COLLECTION {
                    ...
                    if api_token.project_id.is_some() {
                        let is_collection_in_project = collections
                            .filter(
                                crate::database::schema::collections::dsl::id.eq(
                                    req_ctx.resource_id
                                )
                            )
                            .filter(
                                crate::database::schema::collections::dsl::project_id.eq(
                                    api_token.project_id.unwrap_or_default()
                                )
                            )
                ...
                if req_ctx.resource_type == Resources::PROJECT {
                    ...
                    if api_token.project_id.is_none() && api_token.collection_id.is_none() {
                        let user_permissions_option = user_permissions
                            ...
                            .filter(
                                crate::database::schema::user_permissions::dsl::project_id.eq(
                                    req_ctx.resource_id
                                )
                            )
                            .first::<UserPermission>(conn)
        ...
        match creator_uid {
            Some(uid) => Ok(uid),
            None => Err(ArunaError::AuthorizationError(AuthorizationError::PERMISSIONDENIED)),
        }
    }
    ...
    pub fn get_oidc_user(&self, oidc_id: &str) -> Result<Option<uuid::Uuid>, ArunaError> {
        ...
        let result = self.pg_connection
            ...
            .transaction::<_, Error, _>(|conn| {
                users.filter(external_id.eq(oidc_id)).first::<User>(conn).optional()
            })?;

        match result {
            Some(u) => {
                if !u.active {
                    Err(ArunaError::InvalidRequest("User is not activated".to_string()))
                } else {
                    Ok(Some(u.id))
                }
            }
