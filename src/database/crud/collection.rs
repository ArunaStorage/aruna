...
impl Database {
    ...
    pub fn get_collections(
        ...
        let ret_collections = self.pg_connection.get()?.transaction::<Option<
            Vec<(
                Collection,
                Option<Vec<CollectionKeyValue>>,
                Option<Vec<RequiredLabel>>,
            ...
            if let Some(q_colls) = query_collections {
                for col in q_colls {
                    ...
                    if let Some(cl_version) = col.version_id {
                        ...
                        return_vec.push((col, collection_key_values, req_labels, stats, None));
                    }
                }
                Ok(Some(return_vec))
            } else {
...
fn map_to_collection_overview(
    ...
    if let Some(ret_coll) = coll_infos {
        ...
        let label_ont = ret_coll.2.map(|req_labels| LabelOntology {
        ...
        let is_public = match ret_coll.0.dataclass {
            Some(dclass) => matches!(dclass, models::enums::Dataclass::PUBLIC),
            None => false,
        };
